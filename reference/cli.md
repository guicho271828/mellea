...
| `--backend` | ollama \| openai \| rits | `ollama` | Backend used for inference. Options: "ollama", "openai", and "rits". |
| `--backend-req-timeout` | integer | `300` | Timeout in seconds for backend requests. Defaults to "300". |
| `--backend-endpoint` | text | — | Backend endpoint / base URL. Required for "openai" and "rits". |
| `--backend-api-key` | text | — | Backend API key. Required for "openai" and "rits". |
...
>
> - Mellea installed (`uv add mellea`).
> - The target script must define at least one generative function.
